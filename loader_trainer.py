...
# create a list containing the names of the models :
    ...
    name_NBTR='transformer_nhi1_{}'.format(storage)
    name_NBFC='transformer_nhi2_{}'.format(storage)
    name_TR='transformer_nhi3_{}'.format(storage)
    name_TRT2V='transformer_nhi4_{}'.format(storage)
...
#    models.append(model)
    model=TransformerModel(ninp, nhead=1, nhid=64, nlayers=2, backast_size=backcast_length, forecast_size=forecast_length, dropout=0.2, t2v=False, device=device)
    ...
    model=TransformerModel(ninp, nhead=3, nhid=64, nlayers=2, backast_size=backcast_length, forecast_size=forecast_length, dropout=0.2, t2v=False, device=device)
    models.append(model)
    model=TransformerModel(ninp, nhead=4, nhid=64, nlayers=2, backast_size=backcast_length, forecast_size=forecast_length, dropout=0.2, t2v=False, device=device)
    models.append(model)
